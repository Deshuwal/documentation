...
export class Manage_companiesComponent implements OnInit {
	...
	onPageChange(pageInfo) {
		...
		this.dl.doGet(url).subscribe(
			(res: any) => {
				...
				console.log({ real: this.assessments });
	...
	openLinkCompanyModal(content, item) {
		if (item) {
			this.viewedCompany = this.companies.find((company) => company.id == item);
			this.selectedCompanyId = item.id;
		}
	...
	successlinkExistingCompany() {
		this.toastr.success("Company Linked Successfully!", "Success!", {
			timeOut: 10000,
			closeButton: true,
			progressBar: true,
		});
	...
	linkExistingCompany() {
		...
		if (this.linkCompanyForm.valid) {
			this.loading = true;
			this.dl
				.doPost(`users/link_user_company`, this.linkCompanyForm.value)
				.subscribe(
					(res: any) => {
						this.successlinkExistingCompany();
						this.loading = false;
					},
					(err: any) => {
						console.log(err);
