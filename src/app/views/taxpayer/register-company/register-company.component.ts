...
export class RegisterCompanyComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.breadcrumb.setCrumbItem('dashboardRegisterCompany');
  ...
  fetchIndustries() {
    ...
    let url =  `business/get_business_industries`;
		this.dl
			...
			.subscribe(
				(res: any) => {
          console.log('taxPayer', res);
          this.industries = res.map((industry) => industry.name);
					this.loading = false;
				},
				(err) => {
					this.loading = false;
  ...
  addCompany() {
    ...
    this.dl.doPost('users/admin_add_company', this.addCompanyForm.value).subscribe(
      (res: any) => {
  ...
  successAddingCompany() {
    this.toastr.success("Company Registration Successful!", "Success!", {
      timeOut: 10000,
      closeButton: true,
      progressBar: true,
    });
