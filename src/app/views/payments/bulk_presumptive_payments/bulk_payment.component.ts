...
export class PresumptiveBulkPaymentComponent implements OnInit {
  mdas: any;
  ...
  ngOnInit() {
    ...
    this.breadcrumb.setCrumbItem("presumptivePayment");
  ...
  loadAssessments() {
    ...
    if (["1", "3"].includes(this.userProfile.role)) {
      url = `/assessment/list_bulk_presumptive_for_user/${this.userProfile.id}`;
    }
    ...
    if (this.gatewayForm.valid) {
      if (
        ...
        this.makePaymentForTaxItemViaOpayWebPay(
          this.assessmentForDisplay.bulk_assessment_id
        );
      }
    }
  }
  ...
  makePaymentForTaxItemViaOpayWebPay(assessmentNumber) {
    ...
    let data: any = {};
    let returnUrl = this.getPaymentsReturnUrl(assessmentNumber);
    data.reference_no = this.assessmentForDisplay.bulk_assessment_id;
    data.tax_item = "Plateau State Internal Revenue Service";
    data.return_url = returnUrl;
    data.amount = Number(this.bulkAssessmentTotal) * 100;
      ...
      (res: any) => {
        ...
        if (!this.isCliked) {
          res.forEach((item: any) => {
            ...
            this.bulkAssessmentTotal +=
              +Number(individual.tax / 12).toFixed(2) + individual.dev_levy;
          });
          this.isCliked = true;
        }
