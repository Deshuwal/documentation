...
export class PAYEBulkPaymentHistoryComponent implements OnInit {
	mdas: any;
	...
	ngOnInit() {
		...
		this.breadcrumb.setCrumbItem("payePaymentHistory");
	...
	loadAssessments() {
		...
		if (["1", "3", "4"].includes(this.userProfile.role)) {
			url = "/assessment/list_bulk_paye" + "/1";
		}
		...
		this.dl.doGet(url).subscribe(
			(res: any) => {
				...
				this.filteredItems = this.assessments.map((ass) => ({
					...ass,
					yearOfExpiry: ass.period_to.split("-")[0],
				}));
				console.log("filteredItems assessments ", this.filteredItems);
		...
		this.dl.doGet(url).subscribe(
			(res: any) => {
				...
				res.forEach((item: any) => {
					...
					total +=
						+Number(individual.tax).toFixed(2) +
