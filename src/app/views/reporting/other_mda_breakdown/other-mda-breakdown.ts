...
@Component({
	...
	templateUrl: "./other-mda-breakdown.html",
	styleUrls: ["./other-mda-breakdown.css"],
})
export class OtherMDARevenueBreakdown implements OnInit {
	...
	getPSIRSRevenueBreakdown() {
		let psirs_revenue_breakdown = this.parseUrl('reports/mdas_revenue_breakdown');
		this.loading = true;
	...
	filterByDate(resetPagination: boolean = false) {
		...
		let payment_channels = this.parseUrl('reports/payment_channels');
		let mda_revenue_breakdown = this.parseUrl('reports/mda_revenue_breakdown');
		let selected_mda_id = this.selectedMDA;

		const requests = [
			this.dl.doGet(`${psirs_revenue_breakdown}?${period}&page=${this.pagination.currentPage}&selected_mda_id=${selected_mda_id}`),
			this.dl.doGet(`${payment_channels}?${period}&page=${this.pagination.currentPage}&selected_mda_id=${selected_mda_id}`),
			this.dl.doGet(`${mda_revenue_breakdown}?${period}&page=${this.pagination.currentPage}&selected_mda_id=${selected_mda_id}`),
		];
		...
		forkJoin(requests).subscribe(
			(result: any[]) => {
				...
				this.paymentChannelsSum = result[1].kudi + result[1].opay + result[1].wallet + result[1].nibss + result[1].paydirect;
