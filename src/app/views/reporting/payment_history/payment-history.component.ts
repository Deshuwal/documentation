...
export class PaymentHistoryComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.breadcrumb.setCrumbItem("reportingPaymentsInvididual");
  ...
  taxItemsSelected: any = ["Company", "Individual"];
  ...
  console.log(e);
    if(e.value && e.value.length > 3) 
      {this.payer_type = e.value.toLowerCase(); 
      }

      else { this.payer_type = null;}
  }
  ...
  filterByDate(resetPagination:boolean = true) {
    ...
    let url = "payments/list_completed_for_user/" + this.userProfile.id+"?page="+this.pagination.currentPage; 
    ...
    if (this.taxItemItem) {
      url = `${url}&taxItem_id=${this.taxItemItem}`;
    }


    url = `${url}&payer_type=${this.payer_type}`;
  ...
  exportToExcel() {
    ...
    headers.push("date_approved");
    headers.push("payment_date");
    headers.push("period_from");
    headers.push("period_to"); 
    
    let csv =  this.exporter.getCsVPaymentHistory(this.filteredItems, headers);
